    ...
    public abstract class EntityBase
    {
        ...
        public static async Task<EntityBase> FromRequestEvent(SessionEventArgs e)
        {
                    ...
                    config.Value.httpMethods.Contains(e.HttpClient.Request.Method))
                {
                    try
                    {
                        ...
                        return (EntityBase) TypeDescriptor.CreateInstance(
                            ...
                            argTypes: new[] {typeof(Uri), typeof(string), typeof(Dictionary<string, string>), typeof(byte[]), typeof(Config)},
                            args: new object[] {e.HttpClient.Request.RequestUri, e.HttpClient.Request.Method, headers, requestBodyBytes, config.Value})!;
                    }
            ...
            return new RequestErrorEntity("S2000", $"API Not Implemented:\n\n{apiPath}",
                e.HttpClient.Request.RequestUri, e.HttpClient.Request.Method, new Config(apiPath, e.HttpClient.Request.Method)); // error type
        }
        ...
        public EntityBase(Uri requestUri, string httpMethod, Dictionary<string, string> requestHeaders, byte[] requestBody, Config config)
        {
        ...
        public static readonly Dictionary<Type, Config> configs = new()
        {
			...
			//	new Config("/my/regulation_version", string.Empty, 0)
			//},
			//{
			//	typeof(HeaderEntity),
			...
			//{
			//	typeof(PushTokenEnity),
			//	new Config("/my/push_token", string.Empty, 0)
			//},
			//{
